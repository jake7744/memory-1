...
def store_event(
    ...
    if not date:
        date = current["date"]
    if not time_text and not hour_pillar:
        time_text = current["time_hm"]
    matrix = analyze_matrix(load_config(str(config_path)), date)
    ...
    event = {
        "event_id": str(uuid4()),
        "created_at": now_iso(),
        "created_at_timezone": current["timezone"],
        "date": date,
        "time_text": time_text,
        "hour_pillar": hour_slot["pillar"],
        "hour_range": hour_slot["range_label"],
    ...
    weekly_report = generate_weekly_report(date, feedback_dir)
    return {
        "status": "success",
        "event": event,
